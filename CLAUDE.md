...
- **Fundamentals coverage (quality + valuation factors):** `SECProvider` (no key required) sources gross_margin / operating_margin / debt_to_equity from SEC EDGAR for virtually all US-listed equities. As of PLAN_SEC_VALUATION Phase 3 (2026-07-24), P/E, FCF yield, and EV/EBITDA are also free, keyless, and **single-sourced from SEC EDGAR for every ticker** (annual 10-K basis) — `data_providers.CascadeProvider` always consults SEC and strips FMP's own TTM valuation fields before merging, so the composite never mixes TTM and annual bases; `market_data.derive_valuation_ratios` turns SEC's price-independent components into the finished ratios once the snapshot price is in scope. `FMP_API_KEY`, when set, still sharpens quality (TTM, more current, wins on overlap) for the names it covers and supplies the earnings calendar (EDGAR has no forward calendar) — it no longer contributes to valuation at all. A name still shows N/A when EDGAR lacks the needed XBRL tags (thin ADR/20-F filings, e.g. missing diluted-EPS/share-count concepts) or a vintage-mismatch guard drops a combined metric — verified live: of the tickers previously FMP-valuation-sourced, most keep full coverage under the SEC-only basis, a handful of financials lose EV/EBITDA honestly (banks don't tag `OperatingIncomeLoss` meaningfully), rare names lose it all. See `data_providers.py` / `market_data.py` for the full provider chain.
