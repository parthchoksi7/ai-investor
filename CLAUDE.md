...
- **Fundamentals coverage (quality + valuation factors):** `SECProvider` (no key required) sources gross_margin / operating_margin / debt_to_equity from SEC EDGAR for virtually all US-listed equities. As of PLAN_SEC_VALUATION Phase 2 (2026-07-24), P/E, FCF yield, and EV/EBITDA are also free and keyless: `market_data.derive_valuation_ratios` combines SEC EDGAR's price-independent components (annual 10-K basis) with the snapshot's close price for any ticker `FMP_API_KEY` doesn't cover (FMP-first, SEC fills the gap). Verified live coverage lift on the 2026-07-24 snapshot: 20.0% → 61.1%, climbing toward the ~90%+ SEC quality-coverage level as the alternate-day cache rotates. A name still shows N/A when EDGAR lacks the needed XBRL tags (thin ADR/20-F filings) or a vintage-mismatch guard drops a combined metric. Forward earnings calendar still requires `FMP_API_KEY` (EDGAR has no forward calendar). See `data_providers.py` / `market_data.py` for the full provider chain.
