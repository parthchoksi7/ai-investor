...
`market_data.yml` fires at **four** staggered times — 5:00, 7:00, 8:00 and 8:30 AM EDT (the 5:00 AM head start was added for the Stage D expansion sweep). **GitHub Actions scheduled runs are best-effort and can be delayed by hours or skipped during high load** — all four can be deferred simultaneously. On Jun 9 every run was late; on Jun 11 the job was silently skipped; on **Aug 27-28 2026 every trigger was deferred by ~10 hours**, landing at 3-6 PM ET. If no cron lands before 9:45 AM ET, `market_snapshot.json` is stale → the rebalance gate **SKIP-RETRYs** → no rebalance that day.

> ⚠️ **There is NO "skip if today's snapshot already exists" guard** in `fetch_snapshot.py`, despite what this section claimed until 2026-08-28. Every run does the full fetch. That is **deliberate and load-bearing**, not an oversight: the Stage D sweep advances its cursor once per run at `EXPANSION_BATCH_SIZE = 75`, and there are **290 expansion-only names — exactly four batches**, which is why there are exactly four crons. Adding a skip-if-fresh guard would stretch a full sweep from one day to four. `TestMarketDataConcurrency::test_expansion_sweep_still_needs_every_run` guards that arithmetic; if the universe or batch size ever changes so a sweep needs fewer than four runs, the guard becomes viable again.

**The four triggers must not overlap.** When the ~10-hour deferral bunched them 24-52 min apart (Aug 27-28), concurrent runs shared the single Polygon 5-calls/min budget → 429 storms → runs stretched from 12-40 min to 40-70 min → *more* overlap. Both runs also regenerated all eight committed artifacts, so the loser's `git pull --rebase` conflicted in every one and the push step aborted (3 failed runs; no data lost — the winner's snapshot landed). Fixed 2026-08-28 by a **`concurrency` group** (`cancel-in-progress: false`, `workflow_dispatch` exempt via a per-`run_id` key) plus **`timeout-minutes: 50`** so a wedged run cannot hold the group for GitHub's 6-hour default. Note the group cannot serialize against the **cloud routine**, which also commits `fundamentals_cache.json`, `forecasts_scored.jsonl` and `agent_scorecards.json` — that cross-plane race survives, and fails clean rather than corrupting.

Mitigations (in order of robustness):
1. **Safety dispatch** (still the highest-reliability option): trigger `market_data.yml` manually if no fresh `chore: market snapshot` commit has landed by ~9:15 AM ET on a **Wednesday** (the rebalance day — other days run risk_watch, which needs no snapshot): `gh workflow run market_data.yml --repo parthchoksi7/ai-investor` then confirm the commit is on `main`. A manual dispatch is deliberately exempt from the concurrency group, so it never queues behind — or gets cancelled by — a scheduled run.
2. ~~Move the cron earlier~~ — **Done**: earliest trigger is 5:00 AM EDT, 285 min before the routine.
3. ~~Stop overlapping runs from failing each other~~ — **Done** (2026-08-28): concurrency group + job timeout, above.
4. Longer term: have the routine itself dispatch `market_data.yml` and poll for the fresh commit before running `main.py`, rather than assuming the file is present. This is the only mitigation that actually defeats a multi-hour scheduler deferral.
...
- **Fundamentals coverage (quality + valuation factors):** `SECProvider` (no key required) sources gross_margin / operating_margin / debt_to_equity from SEC EDGAR (annual 10-K basis) for virtually all US-listed equities. As of PLAN_SEC_VALUATION Phase 4 (2026-07-24, plan complete), P/E, FCF yield, and EV/EBITDA are also free, keyless, and **single-sourced from SEC EDGAR for every ticker** — `data_providers.CascadeProvider` always consults SEC and strips FMP's own valuation fields before merging. Flow components (EPS, CFO, capex, D&A, operating income) are **trailing-twelve-months** (sum of the 4 most recent contiguous fiscal quarters — `SECProvider._ttm_ex`, which derives the one quarter XBRL never discloses standalone as `annual − known 3 quarters`, and differences consecutive YTD figures for concepts like CFO that are commonly tagged YTD-only, never standalone); balance components (shares, debt, cash) are the **latest available value from any form** (10-K or 10-Q), not just the latest 10-K. `market_data.derive_valuation_ratios` turns these into the finished ratios once the snapshot price is in scope. `FMP_API_KEY`, when set, still sharpens quality (TTM, more current, wins on overlap) for the names it covers and supplies the earnings calendar (EDGAR has no forward calendar) — it no longer contributes to valuation at all. A name still shows N/A when EDGAR lacks the needed XBRL tags, a vintage-mismatch guard drops a combined metric, or a share count fails the `_MIN_PLAUSIBLE_SHARES` sanity floor (some filers report share counts scaled in millions with no way to detect the scale from the API). Verified live against the full 175-ticker universe: 94.3% valuation coverage, 0 errors. See `data_providers.py` / `market_data.py` for the full provider chain.
